...
class PackageSkel(object):
    ...
    def process_node(self, path):    
        ...
        if path.is_file():
            #This is a file, we just need to rename it
            ...
            #self.logger.debug("File: {}".format(path))
...
class PSkel(object):
    ...
    def visit_template_files(self):
        templates_dir = self.get_templates_dir()

    def get_rename_list(self):
        templates_dir = self.get_templates_dir()
    
        for root,directory,files in os.walk(templates_dir):
            for file in files:
                print(os.path.join(root,file), directory)
...
def make_template():
    ...
    pkel = PackageSkel(package_name, destination_path)

    try:
        #pskel.deploy_template()
        #template_files = pskel.get_rename_list()
        pass
